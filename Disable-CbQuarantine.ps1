...
<#
    .SYNOPSIS
    Disabled quarantine for device within Carbon Black.
    ...
    .EXAMPLE
    Disable-CbQuarantine -ComputerName "tims-pc"
#>
...
function Disable-CbQuarantine {
    ...
    if ($searchResults.num_found -eq '0') {
        ...
        $msg     = "Are you sure you want to unquarantine host $($searchResults.results.name[$selection])?"
        $options = '&Yes', '&No'
            ...
            if ($response -eq 0) {
                try {
                    ...
                    if ($uninstallResults.StatusCode -eq "204") {
                        Write-Output "SUCCESS: Quarantine has been disabled for host $($searchResults.results.name)."
                    } else {
        ...
        #User uninstall confirmation dialog
        $title   = "Unquarantine Host"
        $msg     = "Are you sure you want to unquarantine host $($searchResults.results.name)?"
        $options = '&Yes', '&No'
        $default = 1  # 0=Yes, 1=No
        do {
            $response = $Host.UI.PromptForChoice($title, $msg, $options, $default)

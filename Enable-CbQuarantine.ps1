...
<#
    .SYNOPSIS
    Quarantines device within Carbon Black.
    ...
    .EXAMPLE
    Enable-CbQuarantine -ComputerName "tims-pc"
#>
...
function Enable-CbQuarantine {
    ...
    if ($searchResults.num_found -eq '0') {
        ...
        $msg     = "Are you sure you want to quarantine host $($searchResults.results.name[$selection])?"
        $options = '&Yes', '&No'
            ...
            if ($response -eq 0) {
                try {
                    ...
                    if ($uninstallResults.StatusCode -eq "204") {
                        Write-Output "SUCCESS: Host $($searchResults.results.name) has been Quarantined."
                    } else {
        ...
        #User uninstall confirmation dialog
        $title   = "Quarantine Host"
        $msg     = "Are you sure you want to quarantine host $($searchResults.results.name)?"
        $options = '&Yes', '&No'
        $default = 1  # 0=Yes, 1=No
        do {
            $response = $Host.UI.PromptForChoice($title, $msg, $options, $default)

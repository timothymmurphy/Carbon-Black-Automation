...
<#
    .SYNOPSIS
    Disables bypass mode on device within Carbon Black.
    ...
    .EXAMPLE
    Disable-CbBypass -ComputerName "tims-pc"
#>
...
function Disable-CbBypass {
    ...
    if ($searchResults.num_found -eq '0') {
        ...
        $msg     = "Are you sure you want to disable Bypass mode on host $($searchResults.results.name[$selection])?"
        $options = '&Yes', '&No'
            ...
            if ($response -eq 0) {
                try {
                    ...
                    if ($uninstallResults.StatusCode -eq "204") {
                        Write-Output "SUCCESS: Bypass mode has been disabled on host $($searchResults.results.name)."
                    } else {
        ...
        #User uninstall confirmation dialog
        $title   = "Disable Bypass"
        $msg     = "Are you sure you want to disable Bypass mode on host $($searchResults.results.name)?"
        $options = '&Yes', '&No'
        $default = 1  # 0=Yes, 1=No
        do {
            $response = $Host.UI.PromptForChoice($title, $msg, $options, $default)
